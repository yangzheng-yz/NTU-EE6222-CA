...
class Resnet18LSTM(nn.Module):
	def __init__(self,num_classes, pretrained=True, dr_rate=0.2, rnn_hidden_size=100, rnn_num_layers=1, **kwargs):
		...
		self.num_features=baseModel.fc.in_features
		baseModel.fc=Identity()
		self.baseModel=baseModel
		self.dropout=nn.Dropout(self.dr_rate)
		self.rnn=nn.LSTM(self.num_features, self.rnn_hidden_size, self.rnn_num_layers)
		self.fc1=nn.Linear(self.rnn_hidden_size, self.num_classes)
  
	def forward(self,x):
		b_z,c,ts,h,w=x.shape
		x = x.reshape(b_z, ts, c, h, w)
		ii=0
