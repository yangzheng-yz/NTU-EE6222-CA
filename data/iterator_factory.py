...
def get_arid(data_root='./dataset/ARID',
	...
	train_sampler = sampler.MGSampler(num=clip_length, mode='train')
	...
	train = VideoIter(video_prefix=os.path.join(data_root, 'train_%s' % brighten_method if brighten_method is not None else 'train' ),
	...
	val_sampler = sampler.MGSampler(num=clip_length, mode='train')
	...
	val   = VideoIter(video_prefix=os.path.join(data_root, 'val_%s' % brighten_method if brighten_method is not None else 'val'),
...
def get_arid_flow(data_root='./dataset/ARID',
	...
	train_sampler = sampler.RandomSampling(num=clip_length, interval=train_interval, speed=[1.0, 1.0], seed=(seed+0))
	# train_sampler = sampler.SegmentalSampling(num_per_seg=clip_length, segments=segments, interval=train_interval, fix_cursor=False, shuffle=True, seed=(seed+0))
	train = FlowIter(video_prefix=os.path.join(data_root, 'train_flow_%s' % brighten_method if brighten_method is not None else 'train_flow'),
					  txt_list=os.path.join(data_root, 'train.txt'),
					  sampler=train_sampler,
					  force_gray=True,
	...
	val_sampler = sampler.SequentialSampling(num=clip_length, interval=val_interval, fix_cursor=True, shuffle=True)
	# val_sampler = sampler.SegmentalSampling(num_per_seg=clip_length, segments=segments, interval=val_interval, fix_cursor=True, shuffle=True)
	val   = FlowIter(video_prefix=os.path.join(data_root,'val_flow_%s' % brighten_method if brighten_method is not None else 'val_flow'),
					  txt_list=os.path.join(data_root,'val.txt'),
					  sampler=val_sampler,
					  force_gray=True,
					  flow_transforms=flow_transforms.Compose([
